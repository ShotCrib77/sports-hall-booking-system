...
export default function ReservationCard({ reservation, isPast = false, onCancel }: ReservationCardProps) {
    ...
    return (
        <div
        className={`bg-gray-50 border rounded-2xl px-6 py-5 flex items-center gap-5 transition-shadow hover:shadow-md ${
            isPast ? "border-gray-100 opacity-80" : "border-gray-100"
        }`}
        >
        {/* Date block */}
        <div
            className={`flex flex-col items-center justify-center w-16 h-16 rounded-xl shrink-0 ${
            isPast ? "bg-gray-50 border" : "bg-white border"
            }`}
        >
            <span
            className={`text-[10px] font-bold uppercase tracking-widest ${
                isPast ? "text-gray-400" : "text-green-500"
            }`}
            >
            {month}
            </span>
        ...
        <div className="flex-1 min-w-0 flex items-center gap-4">
            <div className="flex flex-col items-center gap-">
                ...
                <span
                    className={`font-mono text-sm font-medium px-2.5 py-0.5 rounded-md ${
                    isPast
                        ? "bg-gray-100 text-gray-400"
        ...
        {(isPast && reservation.booking_status === "no_show") && (
            <div
                className="text-sm font-semibold text-red-500 px-3.5 py-1.5"
            >
                Missed
            </div>
        )}
