    ...
    return (
        <header className="bg-white border-b-2 h-20 text-xl flex items-center px-8 text-gray-800">
            <div className="flex items-center">
                ...
                {userRole === "admin" && 
                    <div className="flex items-center">
                        <div className="w-px h-12 mx-4 md:bg-gray-800 bg-transparent"/> 
                        <h2 className="text-xl xl:text-2xl">Admin</h2>
                    </div>
                }
            </div>

            <nav className="hidden lg:flex items-center ml-auto text-lg">
                <div className="px-4 py-1 border-r border-gray-400 mr-6 gap-4 xl:gap-8 flex">
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="absolute top-20 left-0 right-0 bg-white border-b-2 flex flex-col px-8 py-4 gap-4 z-50 "
                    >
                        ...
                        {userRole === "admin" && 
                            <div className="flex flex-col gap-4">
                                <Link href="/admin/bookings" className={`${isActive("/admin/bookings") ? "font-bold" : "font-normal text-gray-500"}`}>Admin bookings</Link>
                                <Link href="/admin/stats" className={`${isActive("/admin/stats") ? "font-bold" : "font-normal text-gray-500"}`}>Admin stats</Link>
                            </div>
                        }
                        {loading ? <p>Loading...</p> : (isLoggedIn ? <LogoutButton /> : <LoginButton />)}
                    </motion.div>
